...
function onBranch(node, children) {
    ...
    if (node.text == '-') {
        todo()
        const v = {type: "__array__", value: onArrayItem(children)}
        return v
    }
    const callback = (child) => {
        const t = child.__type__
        if (t == "array") {
            if (!items) {
                items = []
            }
            ...
            Object.assign(dict, child)
        }
    }

    children.forEach(callback)
    const key = backspace(node.text)
    let payload = dict
    if (items) {
        payload = items
    }
...
function onLeaf(node) {
   let  s = node.text
    if (s.startsWith('- ')) {
        s = s.replace(/^- +/, '')
        const value = smallify(s.split(/,/).map(trim).map(toArgument))
        return {
            __type__: 'array',
            value
        }
    } else {
        const [key, value] = colonAttr(s)
        return {
            __type__: 'object',
            key,
            value,
...
function onRoot(node, children) {
    ...
    const callback = (child) => {
        ...
        if (t == "array") {
            ...
            store[child.key] = child.value
        } 
        else {
            Object.assign(store, child)
        }
    }
    children.forEach(callback)
    return store
