...
function foobar(node) {
    function runner(node, componentSpec) {
        ...
        for (let i = 0; i < node.children.length; i++) {
            ...
            if (isAttribute(child.text)) {
                const k = componentSpec && match(child.text, /\w+/)
                if (k && k in componentSpec.fields) {
                    const foo = build(child, componentSpec)
                    contentAttrs[k] = foo
                    continue
                } else if (!touchedText) {
                    Object.assign(attrs, lopper(child))
                    continue
                }
            }

            if (child.isBranch()) {
                if (child.text in components) {
                    const spec = components[child.text]
                    const p = runner(child, spec)
                    // console.log("p", p)
                    const q = applyComponentSpec(p, components[child.text], child)
                    // console.log(q)
                    // throw ''
                    children.push(q)
                } else {
                    // console.log(child)
                    const p = accumulateText(child)
                    children.push({text: p, accumulated: true})
                    // i am close ...
                }
            } else {
                touchedText = true
...
const onLeaf = (node) => {
    return node.data()
    return node.text + "\n".repeat(node.newlines)
}

const onBranchEnter = (node) => {
    return 
    const s = node.text
    if (s == 'foobar') {
        return onFoobar
...
const onRoot = (node, children) => {
    return smallify(children)
    return children.join('\n')
}

export const doFoobar = factory({onLeaf, onBranch, onRoot, onBranchEnter})
