...
import { accumulateText } from "/home/kdog3682/2024-javascript/ttt/accumulateText.js"
import { createRoot } from "/home/kdog3682/2024-javascript/ttt/createRoot.js"
import { layouts, mappings, onComponent } from "/home/kdog3682/2024-javascript/ttt/functions.js"
...
function apply(node, opts = {}) {
    const configs = opts.configs || baseConfigs
    ...
    for (const config of configs) {
        ...
        const value = config.callback(node, config, text, apply)
        // NOTE: apply
        // apply is importantly passed to the callback
        // it is what permits recursion inside of the callback
        // if the callback so chooses
    ...
    if (opts.accumulate) {
        return accumulateText(node)
    }

    throw new Error(
        "no value was returned " + drawBorder(node.text),
    )
}
