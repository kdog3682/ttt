...
function foobar(node) {
    if (node.isLeaf()) {
        const keys = [ "ind", "text", "newlines" ]
        return node.data(keys)
    }
}

function funcSplit(items, fn, includeBefore = true) {
    const store = []
    let temp = []
...
function flex(children, apply) {
    const groups = funcSplit(children, (x) => x.endingNewlines)
    const mapped = groups.map((group) =>
        group.map((el) => apply(el))
    )
    return {
        children: mapped,
    }
...
function onComponent(node, config, key, apply) {
    const { children, attrs } = extractAttrs(node)
    ...
    if (config.layouts.hasOwnProperty(key)) {
        return set(config.layouts[key](children, apply))
    }

    if (config.mappings.hasOwnProperty(key)) {

        const runner = (child) => {
            return config.mappings[key](child) 
                || apply(child, { accumulate: true })
        }

        return set(children.map(runner))
    }
}
